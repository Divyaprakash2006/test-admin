...
import { HiBell, HiOutlineBell, HiCheckCircle, HiBolt, HiUsers, HiBookOpen, HiTrash } from 'react-icons/hi2';
...
export default function NotificationBell() {
  ...
  const getIcon = (type) => {
    switch (type) {
      case 'student_joined': return <HiUsers className="w-5 h-5 text-indigo-500" />;
      case 'test_created': return <HiBookOpen className="w-5 h-5 text-emerald-500" />;
      default: return <HiBolt className="w-5 h-5 text-amber-500" />;
    }
  };
      ...
      {isOpen && (
        <div className={`absolute right-0 mt-3 w-80 sm:w-96 rounded-2xl shadow-2xl overflow-hidden z-50 border transform origin-top-right transition-all duration-200 ${
          ...
          <div className={`px-4 py-3 border-b flex items-center justify-between ${isDark ? 'border-gray-800' : 'border-gray-100'}`}>
            <h3 className={`font-bold ${isDark ? 'text-white' : 'text-gray-900'}`}>Admin Alerts</h3>
            <div className="flex items-center gap-3">
              ...
              {notifications.length > 0 && (
                <button
                  onClick={() => { if (window.confirm('Clear all alerts?')) { clearNotifications(); setIsOpen(false); } }}
          ...
          <div className="max-h-[400px] overflow-y-auto">
            {notifications.length === 0 ? (
              <div className="py-12 px-4 flex flex-col items-center justify-center gap-3">
                <p className={`text-sm ${isDark ? 'text-gray-500' : 'text-gray-400'}`}>No alerts to show</p>
              </div>
            ) : (
              notifications.map((n) => (
                <div
